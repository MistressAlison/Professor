...
void main()
{
    ...
    for (float j = 0.; j < ANGLENUM; j += 1.)
    {
        ...
        for (float i = -RANGE; i <= RANGE; i += STEP)
        {
            ...
            if (pos2.y < 0. || pos2.x < 0. || pos2.x > u_screenSize.x || pos2.y > u_screenSize.y)
            continue;
    ...
    float r = length(pos - u_screenSize.xy*.5) / u_screenSize.x;
    float vign = 1. - r*r*r;
